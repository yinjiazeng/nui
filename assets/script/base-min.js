    ...
    __define('src/components/highlight/highlight', function () {
        return this.extend('src/core/component', {
            _static: {
                ...
                _getcode: function _getcode(type, text) {
                    return '<code class="' + type + '">' + text + '</code>';
                },
                _getarr: function _getarr(match, code) {
                    var array = [];
                    if (!match) {
                        array.push(code);
                    } else {
                ...
                _comment: function _comment(code) {
                    //多行注释
                    if (/\/\*/.test(code)) {
            ...
            _title: '',
            _template: '<div class="<% className %>">' + '<%if tools%>' + '<div class="tools">' + '<%if tools.copy%>' + '<em class="copy">复制</em>' + '<%/if%>' + '</div>' + '<%/if%>' + '<div class="body">' + '<table>' + '<%each list val key%>' + '<tr>' + '<%if isLine === true%><td class="line" number="<%key+1%>"><%if bsie7%><%key+1%><%/if%></td><%/if%>' + '<td class="code"><%val%></td>' + '</tr>' + '<%/each%>' + '</table>' + '</div>' + '<%if isTitle%>' + '<em class="title"><%title%></em>' + '<%/if%>' + '</div>',
            _events: {
                'click tr': function clickTr(e, elem) {
                ...
                Nui.each(array, function (v) {
                    if (Nui.trim(v)) {
                        ...
                        if (/^\s*\/\*/.test(v)) {
                            ...
                            //匹配属性
                            if (/\}\s*$/.test(v)) {
                                v = v.replace(/(\s*)([^:;\{\}\/\*]+)(:)([^:;\{\}\/\*]+)/g, '$1' + _class._getcode('attr', '$2') + '$3' + _class._getcode('string', '$4')).replace(/([\:\;\{\}])/g, _class._getcode('symbol', '$1'));
                            }
                            //选择器
                            else {
                                    v = v.replace(/([^\:\{\}\@\#\s\.]+)/g, _class._getcode('selector', '$1')).replace(/([\:\{\}\@\#\.])/g, _class._getcode('symbol', '$1'));
                                }
                        }
                    }
                    str += v;
                    ...
                    if ($.trim(v)) {
                        ...
                        //多行注释
                                ...
                                //关键字、符号、单词
                                        ...
                                        v = v.replace(new RegExp('(' + symbol + ')', 'g'), _class._getcode('symbol', '$1')).replace(new RegExp('(' + kws + ')(\\s+|\\\<code)', 'g'), _class._getcode('keyword', '$1') + '$2').replace(/(\/code>\s*)(\d+)/g, '$1' + _class._getcode('number', '$2')).replace(/(\/code>\s*)?([^<>\s]+)(\s*<code)/g, '$1' + _class._getcode('word', '$2') + '$3');
                                    }
                                v = _class._comment(v);
                            }
                    }
                    ...
                    Nui.each(v1, function (v2, k2) {
                        if (Nui.trim(v2)) {
                            if (k2 == 0) {
                                var istag = false;
                                if (/^\s*\//.test(v2)) {
                                    v2 = v2.replace(/([^\r\n\/]+)/g, _class._getcode('tag', '$1')).replace(/^(\s*\/+)/, _class._getcode('symbol', '$1'));
                                } else {
                                    var preBlank = v2.match(/^\s+/) || '';
                                    if (/\=\s*['"]$/.test(v2)) {
                                        istag = true;
                                    }
                                    v2 = v2.replace(/^\s+/, '').replace(/(\s+)([^'"\/\s\=]+)((\s*=\s*)(['"]?[^'"]*['"]?))?/g, '$1' + _class._getcode('attr', '$2') + _class._getcode('symbol', '$4') + _class._getcode('string', '$5')).replace(/<code class="\w+">(\s*((<<\s*![-\s]+)|([-\s]+>>))?)<\/code>/g, '$1').replace(/^([^\s]+)/, _class._getcode('tag', '$1')).replace(/(\/+\s*)$/, _class._getcode('symbol', '$1'));
                                    v2 = preBlank + v2;
                                }
