...
;(function (__define__) {
    __define__('pages/components/router/script/options', function () {
        return {
            text: '1111',
            color: '#f60'
        };
    ...
    __define__('src/components/router', ['src/core/component', 'src/core/template', 'src/core/events'], function (component, template, events) {
        var statics = {
            _paths: {},
            _init: function _init() {
                var self = this;
            ...
            _setpaths: function _setpaths(rule, paths) {
                if (!this._paths[rule]) {
                    this._paths[rule] = paths;
                }
            },
            _replace: function _replace(hash) {
            ...
            _getWrapper: function _getWrapper(container) {
                return $('<div class="nui-router-wrapper"></div>').appendTo(container);
            },
            _split: function _split(url) {
                var ret = {
                    url: url,
                    params: {}
                };
            ...
            _bindHashchange: function _bindHashchange() {
                var self = this;
                if (Nui.bsie7) {
                    var hashchange = function hashchange(ret) {
            ...
            location: function location(url, data, render) {
                var self = this;
                if (url) {
                    ...
                    Nui.each(this._paths, function (val, rule) {
                        if (rule === url || url.indexOf(val.path) === 0 && (temp = url.replace(new RegExp('^' + val.path), '').replace(/^\//, '')) && temp.split('/').length === val.params.length) {
            ...
            forward: function forward(index) {
        ...
        return this.extend(component, {
            ...
            _render: function _render(url) {
                var self = this,
                    opts = self._options,
                    href = url instanceof jQuery ? url.attr('href') : url;
                if (href) {
                    var trigger = false;
                    var change = function change(callback) {
                        trigger = true;
    ...
    __define__('src/components/placeholder', ['src/core/component'], function (component) {
        return this.extend(component, {
            _options: {
                /**
            ...
            _createRules: function _createRules() {
                var self = this;
    ...
    __define__('./script/demo', function () {
        ...
        router({
            ...
            level: 2,
            template: {
                list: '<ul>' + '<%each list%>' + '<li><a href="<%$value.url%>/<%$value.title%>" class="news"><%$value.title%></a></li>' + '<%/each%>' + '</ul>',
                detail: '<div>' + '<h3><%params.title%></h3>' + '<p>这是<%params.title%>详情，id是<%params.id%>。<input type="text" data-placeholder-options="' + placeholder_opts.id + '"></p>' + '</div>'
            },
            data: {
                list: [{
                    url: '/news/1',
