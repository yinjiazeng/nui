...
;(function (__define) {
    ...
    */
    ...
    __define('src/core/util', {
         ...
         */
        toFixed: function toFixed(digit, decimal, number) {
         ...
         */
        getParam: function getParam(name, urls) {
            var url = decodeURI(urls || location.href),
                value = {};
            startIndex = url.indexOf('?');
            if (startIndex++ > 0) {
                var param = url.substr(startIndex).split('&'),
                    temp;
         ...
         */
        setParam: function setParam(name, value, urls) {
            var self = this,
                url;
         ...
         */
        supportHtml5: function supportHtml5(attr, element) {
         ...
         */
        location: function location(url, target) {
            if (url) {
                jQuery('<a href="' + url + '"' + (target ? 'target="' + (target || '_self') + '"' : '') + '><span></span></a>').appendTo('body').children().click().end().remove();
            }
        },
         ...
         * }
         */
        getData: function getData(element, item, field) {
            var that = this;
            var data = {
                'result': {},
                'voids': 0, //字段中空值数量
         ...
         */
        getFocusIndex: function getFocusIndex(element) {
         ...
         * @return <Boolean>
         */
        isTextSelect: function isTextSelect() {
            var text = '';
            //ie10以及以下浏览器
         ...
         * @return <Boolean>
         */
        isInstallPDF: function isInstallPDF() {
            var i, len;
         ...
         */
        isInstallFlash: function isInstallFlash() {
         ...
         * @return <String>
         */
        numberToCN: function numberToCN(number) {
            //汉字的数字
    ...
    __define('src/core/events', function () {
        return function (opts) {
            ...
            var evt, ele, ret;
            var callback = function callback(e, elem, cbs) {
    ...
    __define('src/core/template', ['src/core/util'], function (util) {
        ...
        var replaceInclude = function replaceInclude(tpl, openTag, closeTag, opts) {
            var that = this;
        ...
        var isNode = (typeof HTMLElement === 'undefined' ? 'undefined' : _typeof(HTMLElement)) === 'object' ? function (obj) {
        ...
        var render = function render(tpl, data, opts) {
            var that = this;
            if (typeof tpl === 'string') {
                opts = opts || {};
                var openTag = opts.openTag || options.openTag,
                    closeTag = opts.closeTag || options.closeTag;
        ...
        var error = function error(code, data, tplid) {
        ...
        var compile = function compile(tpl, logic) {
            if (!tpl) {
                return '';
            }
            var code, res;
        ...
        //判断变量是否存在
        //a.b??  a[b]??  a['b']??  a[b['c']]??
        var exists = function exists(code, isVal) {
        ...
        var match = function match(str, syntax, regexp) {
            var replace;
    ...
    __define('src/core/component', ['src/core/template', 'src/core/events'], function (tpl, events) {
        ...
        var callMethod = function callMethod(method, args, obj) {
            //实参大于形参，最后一个实参表示id
        ...
        var statics = {
            ...
            */
            __setMethod: function __setMethod(apis, components) {
                var self = this;
            ...
            _init: jQuery.noop,
            _jquery: function _jquery(elem) {
                if (elem instanceof jQuery) {
                    return elem;
                }
                return jQuery(elem);
            },
            _getSize: function _getSize(selector, dir, attr) {
                var size = 0;
                attr = attr || 'border';
                dir = dir || 'tb';
                if (attr === 'all') {
                    return this._getSize(selector, dir) + this._getSize(selector, dir, 'padding') + this._getSize(selector, dir, 'margin');
                }
                var group = {
                    l: ['Left'],
                    r: ['Right'],
            ...
            _$ready: function _$ready(name, mod) {
            ...
            hasInstance: function hasInstance(id) {
                var exist = false;
        ...
        return {
            ...
            _bindComponentName: function _bindComponentName(element) {
                var self = this,
            ...
            _on: function _on(type, dalegate, selector, callback, trigger) {
                var self = this;
            ...
            _tpl2html: function _tpl2html(id, data) {
                var opts = {
                    openTag: '<%',
                    closeTag: '%>'
                };
            ...
            _callback: function _callback(method, args) {
                var self = this,
                    opts = self._options;
            ...
            option: function option(opts, isOriginal) {
                var args = arguments;
                var isdef = false;
                var options;
                if (args[0] === true) {
    ...
    __define('src/components/datagrid', function () {
        ...
        return module.extend(component, {
            _static: {
                ...
                _hasChildren: function _hasChildren(value) {
                    return Nui.isArray(value.children) && value.children.length;
                },
                //获取合并单元格数
                _colspan: function _colspan(array, count) {
                    var self = this;
                    if (count === undefined) {
                        count = 0;
                    }
            ...
            _create: function _create() {
                ...
                self.element = self._bindComponentName($(self._tpl2html('layout', self._tplData({
                    ...
                    paging: opts.paging && _typeof(opts.paging) === 'object' && opts.paging.isPage !== false,
                    footer: opts.footer,
            ...
            //获取表格标题行数
            _getRowNumber: function _getRowNumber(array, index, arr, cellid, parent) {
                var self = this,
                    _class = self.constructor;
                if (!arr[index]) {
                    arr[index] = true;
                    ...
                    if (v.length) {
                        if (self.list.length && typeof opts.rowRender === 'function') {
                            ...
                            rowHtml = self._tpl2html('rows', {
                                ...
                                rowRender: function rowRender(val, i) {
            ...
            _checkradio: function _checkradio() {
                var self = this,
                    opts = self._options;
                var callback = function callback(elem, e) {
            ...
            _resetSize: function _resetSize() {
                ...
                if (opts.height !== 'auto') {
                    ...
                    var height = conntailerHeight - self._tableAllTitle.outerHeight() - _class._getSize(self._tableAllTitle, 'tb', 'margin') - self._foot.outerHeight() - _class._getSize(self._foot, 'tb', 'margin');
            ...
            _rowHeight: function _rowHeight() {
                var self = this;
                if (self._hasLeftRight) {
            ...
            _setRowCol: function _setRowCol(array, type, row) {
                var self = this,
                    opts = self._options,
                    _class = self.constructor;
                if (row === undefined) {
            ...
            _editInput: function _editInput(e, elem) {
                var dom = elem.get(0);
            ...
            _horzFocus: function _horzFocus(e, elem, type, isTab) {
            ...
            _verticalFocus: function _verticalFocus(e, elem, type) {
                var td = elem.closest('td.table-cell');
                var index = td.index();
            ...
            _dirFocus: function _dirFocus(e, elem) {
                var self = this,
                    keycode = e.keyCode;
            ...
            _events: {
                ...
                'mouseenter .table-tbody .table-row': function mouseenterTableTbodyTableRow(e, elem) {
                ...
                'mouseleave .table-tbody .table-row': function mouseleaveTableTbodyTableRow(e, elem) {
                    if (this._tableFixed.length) {
            ...
            _enable: function _enable(e, elem) {
                if (elem.prop('readonly') || elem.prop('disabled')) {
                    e.stopPropagation();
                    return false;
                }
            },
            _active: function _active(e, elem, data) {
                var self = this;
            ...
            _rowdblclick: function _rowdblclick(e, elem, data) {
                return this._callback('RowDblclick', arguments);
            },
            _scroll: function _scroll(elem) {
                var self = this;
            ...
            checkedData: function checkedData(field) {
                var self = this;
                var data = [];
                ...
                if (!attr || $.isPlainObject(attr)) {
                    ...
                    return this.each(function () {
                        ...
                        if (checkradio.children().attr('checkname')) {
                            ...
                            checkradio.wrapInner('<em class="ui-' + type + checked + disabled + '" checkname="' + name + '"></em>').children().click(function (e) {
                                var ele = $(this);
    ...
    __define('pages/components/datagrid/script/paging', function (require, imports, renders, extend, exports) {
        ...
        ;!function (window, document, $, undefined) {
            ...
            var request = function request(op) {
                op.dataType = 'json';
                return $.ajax(op);
            };
            ...
            Paging.prototype = {
                ...
                jump: function jump(page) {
                    var that = this,
                        count = Math.ceil(that.aCount / that.pCount),
                        current;
                    that.showload = true;
                    if (that.aCount > 0) {
                ...
                echoList: function echoList(html, i, instance) {
                    var that = this;
                    if (that.current == i) {
                ...
                create: function create() {
                    var that = this,
                        ...
                        current = that.current,
                        html = '',
                        next = count == current ? 1 : current + 1,
                        instance = that.instance(),
                        extPage = that.extPage;

                    if (extPage.wrap) {
                        var page = '<div>';
                        page += current == count || count == 0 ? '<span>' + extPage.next + '</span>' : '<a href="javascript:' + instance + '.jump(' + (current + 1) + ');" target="_self">' + extPage.next + '</a>';
                        page += current == 1 ? '<span>' + extPage.prev + '</span>' : '<a href="javascript:' + instance + '.jump(' + (current - 1) + ');" target="_self">' + extPage.prev + '</a>';
            ...
            $.extend({
                paging: function paging(name, options) {
    ...
    __define('./script/demo', function (require, imports, renders, extend, exports) {
        ...
        var a = datagrid({
            ...
            rowRender: function rowRender(self, list, v, k) {
                return template.render(renders('' + '' + '<%each list%>' + '' + '<tr class="table-row">' + '' + '<%each cols col%>' + '' + '<td class="table-cell" width="<%col.width%>">' + '' + '</td>' + '' + '<%/each%>' + '' + '</tr>' + '' + '<%/each%>' + '' + ''), {
                    cols: v,
                    list: list
                });
            }
