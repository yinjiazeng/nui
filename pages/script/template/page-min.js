    ...
    __define__('./script/template/page', ['src/core/template'], function (tpl) {
        ...
        $('#start').click(function () {
            ...
            run('nuiTemplate', function () {
                tpl.render(renders('' + '' + '<%each list val key%>' + '' + '<%each val v k%>' + '' + '<%k%>:<%v%>' + '' + '<%/each%>' + '' + '<%/each%>' + '' + ''), list);
            }, function () {
                run('dot', function () {
                    doT.compile(renders('' + '' + '{{~it.list :val:key}}' + '' + '{{ for(var k in val) { }}' + '' + '{{=k}}:{{=val[k]}}' + '' + '{{ } }}' + '' + '{{~}}' + '' + ''))(list);
                }, function () {
                    run('artTemplate', function () {
                        template.render(renders('' + '' + '{{each list as val key}}' + '' + '{{each val as v k}}' + '' + '{{k}}:{{v}}' + '' + '{{/each}}' + '' + '{{/each}}' + '' + ''))(list);
                    }, function () {
                        run('baiduTemplate', function () {
                            baidu.template(renders('' + '' + '<%for(var i=0;i<list.length;i++){%>' + '' + '<%for(var j in list[i]){%>' + '' + '<%=j%>:<%=list[i][j]%>' + '' + '<%}%>' + '' + '<%}%>' + '' + ''), list);
                        }, function () {
                            run('juicer', function () {
                                juicer(renders('' + '' + '{@each list as item, key}' + '' + '{@each item as v, k}' + '' + '${k}:${v}' + '' + '{@/each}' + '' + '{@/each}' + '' + ''), list);
                            }, function () {
                                run('handlebars', function () {
                                    Handlebars.compile(renders('' + '' + '{{#each list}}' + '' + '{{#each this}}' + '' + '{{@key}}:{{this}}' + '' + '{{/each}}' + '' + '{{/each}}' + '' + ''))(list);
                                }, function () {
                                    btn.show();
                                });
                            });
