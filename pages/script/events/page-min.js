...
;(function (__define__) {
    ...
    __define__('src/components/layer/layer', ['src/core/component', 'src/core/util', 'src/core/template'], function (component, util, template) {
        ...
        return this.extend(component, {
            ...
            _options: options,
            _template: {
                layout: '<div class="<% className %>" style="<% include \'style\' %>">' + '<div class="layer-box">' + '<%if close%>' + '<% var btn = close %>' + '<% include "button" %>' + '<%/if%>' + '<%if bubble%>' + '<span class="layer-bubble layer-bubble-<%bubble.dir||"top"%>"' + '<%if bubble.style%>' + ' style="<%each bubble.style v n%><%n%>:<%v%>;<%/each%>"' + '<%/if%>' + '><b></b><i></i></span>' + '<%/if%>' + '<%if title%>' + '<div class="layer-head">' + '<span class="layer-title"><%title%></span>' + '</div>' + '<%/if%>' + '<div class="layer-body">' + '<div class="layer-main">' + '<%content%>' + '</div>' + '</div>' + '<%if button && button.length%>' + '<div class="layer-foot" style="text-align:<%align%>">' + '<div class="layer-inner">' + '<%each button btn%>' + '<%include "button"%>' + '<%/each%>' + '</div>' + '</div>' + '<%/if%>' + '</div>' + '</div>',
                button: '<button class="ui-button' + '<%if btn.name%>' + '<%each [].concat(btn.name) name%> ui-button-<%name%><%/each%>' + '<%/if%> layer-button-<%btn.id%>"' + '<%if btn.style%>' + ' style="<%each btn.style v n%><%n%>:<%v%>;<%/each%>"' + '<%/if%>><%btn.text || "按钮"%></button>',
                iframe: '<iframe<%each attr%> <%$index%>="<%$value%>"<%/each%>></iframe>',
                mask: '<div class="nui-layer-mask' + '<%if skin%> nui-layer-mask-<%skin%><%/if%>" style="<%include \'style\'%>">' + '<div class="layer-mask"></div>' + '</div>',
                movemask: '<div class="nui-layer-movemask' + '<%if skin%> nui-layer-movemask-<%skin%><%/if%>" style="<%include \'style\'%>">' + '</div>',
            ...
            */
            ...
            _create: function _create() {
                var self = this,
                    opts = self._options;
                var buttons = self._createButton(),
                    isTitle = false;
                if (opts.isTips !== true) {
            ...
            _iframeOnload: function _iframeOnload() {
                var self = this;
            ...
            _createButton: function _createButton() {
                var self = this,
                    ...
                    buttons = {},
                    caches = {},
                    isTips = opts.isTips === true;
                var add = function add(id, btn) {
                ...
                if (self._button[0] && self._button[0].id === 'close') {
                    buttons.close = self._button[0], buttons.button = self._button.slice(1);
                } else {
            ...
            _buttonEvent: function _buttonEvent() {
                var self = this,
                    opts = self._options;
            ...
            _stopProp: function _stopProp() {
                this._on('click', this.element, function (e) {
                    e.stopPropagation();
                });
            },
            _bindTop: function _bindTop() {
                var self = this;
                self._on('click', self.element, function () {
                    self._setzIndex();
                });
            },
            _bindMove: function _bindMove() {
                var self = this,
                    opts = self._options,
                ...
                self._on('mousemove', Nui.doc, function (e) {
                    var width = self._container.outerWidth(),
                        height = self._container.outerHeight();
                    if (isMove) {
                        _x = e.pageX - x;
                        _y = e.pageY - y;
                        _x < 0 && (_x = 0);
            ...
            _setLower: function _setLower(destroy) {
                var self = this,
                    _class = self.constructor,
                    opts = self._options,
                    unders = [];
            ...
            _setSize: function _setSize() {
                ...
                var edgeSize = _class._getSize(self._box, 'tb', 'all') + self.head.outerHeight() + _class._getSize(self.head, 'tb', 'margin') + _class._getSize(self._body, 'tb', 'all') + self.foot.outerHeight() + _class._getSize(self.foot, 'tb', 'margin');
    ...
    __define__('src/components/layer/confirm', ['src/components/layer/layer'], function (layer) {
    ...
    __define__('./script/events/page', ['src/core/template', 'src/core/events', 'src/components/layer/confirm'], function (template, events, confirm) {
        var evt = events({
            elem: $('#data'),
            data: ['蔬菜', '水果', '苹果'],
            template: this.renders('' + '' + '<%each $list%>' + '' + '<li class="e-mt5">' + '' + '<label>' + '' + '<input type="checkbox" value="<%$index%>">' + '' + '<span><%$value%></span>' + '' + '</label>' + '' + '</li>' + '' + '<%/each%>' + '' + ''),
            events: {
            ...
            del: function del(e, elem, data) {
                var that = this;
                Nui.each(data, function (v) {
                    delete that.data[v];
                });
            },
            update: function update(e, elem, data) {
                var that = this;
