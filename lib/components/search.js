...
Nui.define(['../core/component', '../core/util'], function(component, util){
    ...
    return this.extend(component, {
                ...
                '<div class="<% className %>"<%if style%> style="<%include \'style\'%>"<%/if%>>'+
                    ...
                    '<div class="search-body">'+
                        ...
                        '<div class="search-inner">'+
                            '<%each tabs%>'+
                                '<div class="search-content<%if $index === 0%> search-result<%/if%>" style="display:none;"></div>'+
                            '<%/each%>'+
                        '</div>'+
                    '</div>'+
                    '<%include "foot"%>'+
            ...
            tabs:
                '<%if tabs.length > 1%>'+
                    '<div class="search-tabs">'+
                        '<%each tabs tab%>'+
                            '<span class="search-tab"<%if $index === 0%> style="display:none;"<%/if%>><%tab.title%></span>'+
                        '<%/each%>'+
                    '</div>'+
                '<%/if%>',
            tag:
                '<span class="nui-tag">'+
                    ...
                    '<%if fields?? && fields%>'+
                        '<%each fields%>'+
                            '<input type="hidden" class="search-hidden" name="<%$index%>" value="<%$value%>">'+
                        '<%/each%>'+
                    '<%/if%>'+
                '</span>'
        },
        _events:{
            ...
            'mouseenter li.search-item':'_searchMouseover _itemMouseover',
            'mouseleave li.search-item':'_itemMouseout',
            'click li.search-item':'_select',
            'click .search-tab':'_toggle'
        },
        ...
        _create:function(){
            ...
            self.$inner = self.$body.children('.search-inner');
            self.$result = self.$inner.children('.search-result');

            self._elemData[0].$elem = $();
            self._elemData[0].$container = self.$result;
            
            if(self._isTab){
                var tabs = self.$body.children('.search-tabs').children();
        ...
        _initTemplate:function(name){
            ...
            if(typeof content !== 'string'){
                if(name === 'item' && opts.field){
                    content = '<li class="search-item<%selected($data)%>" data-index="<%$index%>"><%$data["'+ opts.field +'"]??%></li>'
                }
                else{
                    content = ''
                }
                ...
                content = '<li class="search-item<%selected($data)%>" data-index="<%$index%>">'+ content +'</li>'
            }

            if(name !== 'item' && content){
                content = '<div class="search-'+ name +'">'+ content +'</div>'
            }
